...
class MultiTextAdapter(
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: MultiTextItem) {
            ...
            if (item.placeHolder.isNotBlank()) {
                ...
                if (item.inputType.equals("date", ignoreCase = true)) {
                    if (item.minDate != null || item.maxDate != null) {
                        if (item.minDate != null && item.maxDate != null) {
                            ...
                            Util.showDateDialogEnableBetweenMinMax(fragment.activity, etValue,
                                minDate, maxDate)
                        } else if (item.minDate != null && item.maxDate == null) {
                            val minDate = Util.getDateFromTimestamp(item.minDate, Constants.FORM_DATE)
                            Util.showDateDialogEnableBetweenMinToday(fragment.activity, etValue, minDate)
                        ...
                        if (item.pastAllowedDays != null && item.futureAllowedDays != null) {
                            ...
                            Util.showDateDialogEnableAfterMin(fragment.activity, etValue, pastAllowedDate)
                        } else if (item.futureAllowedDays != null && item.pastAllowedDays == null) {
                            val futureAllowedDate = Util.getPastFutureDateStringFromToday(
                                item.futureAllowedDays,
                                Constants.FORM_DATE
                            )
                            Util.showDateDialogEnableBeforeMax(fragment.activity, etValue, futureAllowedDate)
                        }
                    } else {
